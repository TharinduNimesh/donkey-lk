...
export default function CreateTaskPage() {
  ...
  const loaderSteps = [
    ...
    { text: "Saving task details" },
    { text: "Uploading bank slip" },
    { text: "Finalizing" },
  ];
  ...
  const handleSaveAsDraft = async () => {
    ...
    setLoaderStep(0);
    try {
      setLoaderStep(0); // Uploading content
      // ...existing code for uploading content...
      setLoaderStep(1); // Saving task details
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      ...
      if (form.payment.method === 'bank-transfer') {
        setLoaderStep(2); // Uploading bank slip
