...
import { ArrowLeft, Archive, CheckCircle, Trash2, RotateCcw } from "lucide-react";
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-3 w-full sm:w-auto">
          ...
          {taskStatus === "DRAFT" && onDelete && (
            ...
            >
              ...
              {isLoading && confirmationModal.action === "delete"
                ? "Deleting..."
                : "Delete Task"}
            </Button>
          )}
          ...
          {taskStatus === "ACTIVE" && (
            <Button
              ...
              onClick={() =>
                setConfirmationModal({ isOpen: true, action: "archive" })
              }
              disabled={isLoading}
            >
              <Archive className="mr-2 h-4 w-4" />
              {isLoading && confirmationModal.action === "archive"
                ? "Archiving..."
                : "Archive Task"}
            </Button>
          )}
          ...
          {taskStatus === "ARCHIVED" && onUnarchive && (
            <Button
              ...
              onClick={() =>
                setConfirmationModal({ isOpen: true, action: "unarchive" })
              }
              disabled={isLoading}
            >
              <RotateCcw className="mr-2 h-4 w-4" />
              {isLoading && confirmationModal.action === "unarchive"
                ? "Unarchiving..."
                : "Unarchive Task"}
            </Button>
          )}
          ...
          {taskStatus === "ACTIVE" && (
            <Button
              ...
              onClick={() =>
                setConfirmationModal({ isOpen: true, action: "complete" })
              }
              disabled={isLoading}
            >
              <CheckCircle className="mr-2 h-4 w-4" />
              {isLoading && confirmationModal.action === "complete"
                ? "Completing..."
                : "Mark as Completed"}
            </Button>
          )}
          ...
          {confirmationModal.isOpen &&
            confirmationModal.action === "delete" && (
              <TaskConfirmationModal
                ...
                onClose={() =>
                  setConfirmationModal({ isOpen: false, action: "delete" })
                }
                onConfirm={() => {
          ...
          {confirmationModal.isOpen &&
            confirmationModal.action === "archive" && (
              <TaskConfirmationModal
                isOpen={confirmationModal.isOpen}
                onClose={() =>
                  setConfirmationModal({ isOpen: false, action: "archive" })
                }
                onConfirm={onArchive}
                title="Archive Task"
          ...
          {confirmationModal.isOpen &&
            confirmationModal.action === "unarchive" && (
              <TaskConfirmationModal
                isOpen={confirmationModal.isOpen}
                onClose={() =>
                  setConfirmationModal({ isOpen: false, action: "unarchive" })
                }
                onConfirm={() => {
          ...
          {confirmationModal.isOpen &&
            confirmationModal.action === "complete" && (
              <TaskConfirmationModal
                isOpen={confirmationModal.isOpen}
                onClose={() =>
                  setConfirmationModal({ isOpen: false, action: "complete" })
                }
